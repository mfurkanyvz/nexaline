    ...
    return jsonify(
        {
            ...
            "mobilePlatform": {
                ...
                "androidSessions": DeviceSession.query.filter(
                    DeviceSession.revoked_at.is_(None),
                    DeviceSession.user_agent.ilike("%android%"),
                ).count(),
                "pushSubscriptions": PushSubscription.query.count(),
                "androidPushSubscriptions": PushSubscription.query.filter(PushSubscription.user_agent.ilike("%android%")).count(),
                "vapidReady": bool(ensure_vapid_keys()[0]),
                "capabilities": [
                    {"name": "Cihaz kamerası", "status": "ready", "detail": "capture intent + güvenli FileProvider"},
                    {"name": "Cihaz galerisi", "status": "ready", "detail": "Android belge/fotoğraf seçicisi"},
                    {"name": "Mesaj bildirimleri", "status": "ready", "detail": "kalıcı VAPID Web Push + sesli Android kanalı"},
                    {"name": "Gelen arama", "status": "ready", "detail": "çağrı kanalı, zil, kilit ekranı ve tam ekran intent"},
                    {"name": "Konum", "status": "ready", "detail": "özellik kullanılırken istenen hassas/yaklaşık konum izni"},
                    {"name": "Android performansı", "status": "ready", "detail": "donanım hızlandırma ve hafif native tema"},
                ],
            },
